...
class ServerHttp1StreamHandler implements ResourceHolder {
    ...
    ServerHttp1StreamHandler(
        ...
        this.internalDataChannel = new DataStreamChannel() {
            ...
            public void endStream(final List<? extends Header> trailers) throws IOException {
                ...
                if (requestStateRef.get() == MessageState.COMPLETE && !keepAlive) {
                    outputChannel.close();
                }
            }
        ...
        this.requestStateRef = new AtomicReference<>(MessageState.HEADERS);
        this.responseStateRef = new AtomicReference<>(MessageState.IDLE);
    ...
    private void commitResponse(
        ...
        if (responseCommitted.compareAndSet(false, true)) {
            ...
            if (endStream) {
                ...
                responseStateRef.set(MessageState.BODY);
    ...
    void terminateExchange(final HttpException ex) throws HttpException, IOException {
        if (done.get() || requestStateRef.get() != MessageState.HEADERS) {
    ...
    void consumeHeader(final HttpRequest request, final EntityDetails requestEntityDetails) throws HttpException, IOException {
        ...
        requestStateRef.set(requestEntityDetails == null ? MessageState.COMPLETE : MessageState.BODY);
        try {
    ...
    void consumeData(final ByteBuffer src) throws HttpException, IOException {
        if (done.get() || requestStateRef.get() != MessageState.BODY) {
            throw new ProtocolException("Unexpected message data");
        }
        if (responseStateRef.get() == MessageState.ACK) {
            outputChannel.requestOutput();
        }
    ...
    void dataEnd(final List<? extends Header> trailers) throws HttpException, IOException {
        ...
        requestStateRef.set(MessageState.COMPLETE);
        if (responseStateRef.get() == MessageState.COMPLETE && !keepAlive) {
            outputChannel.close();
        }
