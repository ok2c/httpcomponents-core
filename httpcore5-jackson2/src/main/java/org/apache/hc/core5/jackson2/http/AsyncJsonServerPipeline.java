...
public final class AsyncJsonServerPipeline {
    ...
    public class RequestContentStage {
        ...
        public <T> ResponseStage<T> consume(
                final Resolver<HttpRequest, AsyncRequestConsumer<Message<HttpRequest, T>>> requestConsumerResolver) {
         ...
         * @param <T> request content representation.
         */
        public <T> ResponseStage<T> consumeContent(
                final Resolver<HttpRequest, Supplier<AsyncEntityConsumer<T>>> dataConsumerResolver) {
            return new ResponseStage<>(
                    requestValidator,
                    r ->
                            new BasicRequestConsumer<>(dataConsumerResolver.resolve(r)));
        }
        ...
        public <T> ResponseStage<Message<HttpRequest, T>> asObject(final JavaType javaType) {
            return consume(r -> JsonRequestConsumers.create(objectMapper, javaType));
        }
         ...
         */
        public <T> ResponseStage<T> asObject(final Class<T> clazz) {
            return consume(r -> JsonRequestConsumers.create(objectMapper, clazz));
        }
         ...
         */
        public <T> ResponseStage<T> asObject(final TypeReference<T> typeReference) {
            return consume(r -> JsonRequestConsumers.create(objectMapper, typeReference));
        }

        /**
         * Configures the pipeline to process the incoming request content as a {@link JsonNode} instance.
         */
        public ResponseStage<JsonNode> asJsonNode() {
            return consume(r -> JsonRequestConsumers.create(objectMapper.getFactory()));
        }
        ...
        public ResponseStage<Message<HttpRequest, Void>> ignoreContent() {
            return consume(r -> new BasicRequestConsumer<>(DiscardingEntityConsumer::new));
        }
    ...
    public class ResponseStage<I> {
        ...
        private final Validator<HttpRequest> requestValidator;
        private final Resolver<HttpRequest, AsyncRequestConsumer<Message<HttpRequest, I>>> requestConsumerResolver;

        private ResponseStage(
                final Validator<HttpRequest> requestValidator,
                final Resolver<HttpRequest, AsyncRequestConsumer<Message<HttpRequest, I>>> requestConsumerResolver) {
            this.requestValidator = requestValidator;
    ...
    public class ResponseContentStage<I> {
         ...
         */
        public <O> RequestHandlingStage<I, O> produce(final Resolver<Message<HttpResponse, O>, AsyncResponseProducer> responseProducerResolver) {
         ...
         * @param <O> response content representation.
         */
        public <O> RequestHandlingStage<I, O> produceContent(final Resolver<O, AsyncEntityProducer> dataProducerResolver) {
         ...
         * with the given {@link Class}.
         */
        public <O> RequestHandlingStage<I, O> asObject(final Class<O> clazz) {
         ...
         * Configures the pipeline to represent the response message content as an object.
         */
        public RequestHandlingStage<I, JsonNode> asJsonNode() {
            return produce(m -> JsonResponseProducers.create(m.head(), m.body(), objectMapper));
        }
    ...
    public class RequestHandlingStage<I, O> {
        ...
        private final Validator<HttpRequest> requestValidator;
        private final Resolver<HttpRequest, AsyncRequestConsumer<Message<HttpRequest, I>>> requestConsumerResolver;
        private final Resolver<Message<HttpResponse, O>, AsyncResponseProducer> responseProducerResolver;

        private RequestHandlingStage(
                final Validator<HttpRequest> requestValidator,
                final Resolver<HttpRequest, AsyncRequestConsumer<Message<HttpRequest, I>>> requestConsumerResolver,
                final Resolver<Message<HttpResponse, O>, AsyncResponseProducer> responseProducerResolver) {
            this.requestValidator = requestValidator;
            this.requestConsumerResolver = requestConsumerResolver;
        ...
        public Supplier<AsyncServerExchangeHandler> supplier() {
            return () -> new AbstractServerExchangeHandler<Message<HttpRequest, I>>() {
                ...
                protected AsyncRequestConsumer<Message<HttpRequest, I>> supplyConsumer(
                    ...
                    final AsyncRequestConsumer<Message<HttpRequest, I>> requestConsumer = requestConsumerResolver.resolve(request);
                ...
                protected void handle(
                    ...
                    final Message<HttpResponse, O> responseMessage = exchangeHandler.handle(requestMessage, context);
