...
public final class AsyncJsonServerPipeline {
    ...
    public class RequestContentStage {
        ...
        public <T> ResponseStage<T> consume(
                final Resolver<HttpRequest, AsyncRequestConsumer<T>> requestConsumerResolver) {
        ...
        public <T> ResponseStage<Long> asSequence(
            ...
            return consume(r -> JsonRequestConsumers.create(objectMapper, javaType, validator, resultSink));
        }
        ...
        public <T> ResponseStage<Long> asSequence(
                ...
                final JsonConsumer<HttpRequest> validator,
                final JsonResultSink<T> resultSink) {
            return consume(r -> JsonRequestConsumers.create(objectMapper, clazz, validator, resultSink));
        }
        ...
        public <T> ResponseStage<Long> asSequence(
                ...
                final JsonConsumer<HttpRequest> validator,
                final JsonResultSink<T> resultSink) {
            return consume(r -> JsonRequestConsumers.create(objectMapper, typeReference, validator, resultSink));
        }
        ...
        public ResponseStage<Void> asEvents(
            ...
            return consume(r -> JsonRequestConsumers.create(objectMapper.getFactory(), validator, eventHandler));
        }
    ...
    public class ResponseStage<I> {
        ...
        private final Validator<HttpRequest> requestValidator;
        private final Resolver<HttpRequest, AsyncRequestConsumer<I>> requestConsumerResolver;

        private ResponseStage(
                final Validator<HttpRequest> requestValidator,
                final Resolver<HttpRequest, AsyncRequestConsumer<I>> requestConsumerResolver) {
            this.requestValidator = requestValidator;
        ...
        public Supplier<AsyncServerExchangeHandler> supplier() {
            return () -> new AbstractServerExchangeHandler<I>() {
                ...
                protected AsyncRequestConsumer<I> supplyConsumer(
                    ...
                    final AsyncRequestConsumer<I> requestConsumer = requestConsumerResolver.resolve(request);
